...
* Tags: 
* 
* ==> IT IS THE EXACT COPY OF MODEL 06 FROM PREY PREDATOR TUTORIAL
* Only cosmetic changes, considering experiments for viz & exploration 
* 
*/
...
global {
	...
	init {
		...
		create predator number: nb_preys_init * pp_ratio;
	}
...
species predator parent: generic_species {
	...
	float energy_from_eat {
		...
		if(! empty(reachable_preys)) {
			prey target <- any(reachable_preys);
			float target_energy <- target.energy;
			ask target {do die;} 
...
experiment prey_predator type: gui {
	...
	output {
		...
		display graphic {
			chart "Species evolution" type: series memorize:false {
				data "Preys" value: nb_preys color: #blue;
				data "Predators" value: nb_predators color: #red;
				data "Grass" value: sum(vegetation_cell collect (each.food)) color: #green;
			}
		}
	}
}
